...
export async function sendEmail(payload: EmailPayload): Promise<boolean> {
  ...
  if (type === 'booking_confirmed') {
    ...
    subject = `Lesson Cancelled \u2013 ${courseName} on ${formattedDate}`
    const refundLine = refundKind === 'none' ? '' : ' The points will be automatically returned to your account.'
    ...
    // No figure here on purpose. What confirming costs depends on the invited
    // family's own VIP level and on whether the slot is off-peak, and we would
    // be quoting the inviter's price. The dashboard shows them theirs before
    // they confirm.
    subject = `Invitation: ${inviterName} invited ${studentName} to a 1-on-2 lesson`
    html = `<div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; background: #f9f9f9; padding: 32px; border-radius: 12px;"><div style="text-align: center; margin-bottom: 24px;"><h1 style="color: #1a2744; font-size: 24px; margin: 0;">Manta Shark Aquatics</h1></div><div style="background: white; border-radius: 8px; padding: 24px; margin-bottom: 16px;"><h2 style="color: #1a2744; margin-top: 0;">🔔 Partner Lesson Invitation</h2><p>Hi ${parentName},</p><p><strong>${inviterName}</strong> has invited <strong>${studentName}</strong> to join the lesson below. Please log in to your Dashboard within 15 minutes to confirm:</p><table style="width: 100%; border-collapse: collapse;"><tr><td style="padding: 8px 0; color: #666;">Student</td><td style="padding: 8px 0; font-weight: 600;">${studentName}</td></tr><tr><td style="padding: 8px 0; color: #666;">Course</td><td style="padding: 8px 0; font-weight: 600;">${courseName}</td></tr><tr><td style="padding: 8px 0; color: #666;">Coach</td><td style="padding: 8px 0; font-weight: 600;">${coachName}</td></tr><tr><td style="padding: 8px 0; color: #666;">Date</td><td style="padding: 8px 0; font-weight: 600;">${formattedDate}</td></tr><tr><td style="padding: 8px 0; color: #666;">Time</td><td style="padding: 8px 0; font-weight: 600;">${time}</td></tr></table><div style="text-align: center; margin-top: 24px;"><a href="https://www.mantasharkaquatics.net/dashboard" style="display: inline-block; background: #7b61c4; color: white; font-weight: 700; padding: 14px 32px; border-radius: 8px; text-decoration: none;">Review Invitation</a></div><p style="color: #999; font-size: 12px; margin-top: 16px;">Confirming will use points from your wallet — your dashboard shows exactly how many before you confirm. The invitation expires automatically if not confirmed in time.</p></div></div>`
    ...
    html = `<div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; background: #f9f9f9; padding: 32px; border-radius: 12px;"><div style="text-align: center; margin-bottom: 24px;"><h1 style="color: #1a2744; font-size: 24px; margin: 0;">Manta Shark Aquatics</h1></div><div style="background: white; border-radius: 8px; padding: 24px; margin-bottom: 16px;"><h2 style="color: #1a2744; margin-top: 0;">\u23f3 Invitation Expired</h2><p>Hi ${parentName},</p><p>The partner lesson invitation below was not confirmed in time, so the reserved time has been released. <strong>No points were used.</strong></p><table style="width: 100%; border-collapse: collapse;"><tr><td style="padding: 8px 0; color: #666;">Student</td><td style="padding: 8px 0; font-weight: 600;">${studentName}</td></tr><tr><td style="padding: 8px 0; color: #666;">Course</td><td style="padding: 8px 0; font-weight: 600;">${courseName}</td></tr><tr><td style="padding: 8px 0; color: #666;">Coach</td><td style="padding: 8px 0; font-weight: 600;">${coachName}</td></tr><tr><td style="padding: 8px 0; color: #666;">Date</td><td style="padding: 8px 0; font-weight: 600;">${formattedDate}</td></tr><tr><td style="padding: 8px 0; color: #666;">Time</td><td style="padding: 8px 0; font-weight: 600;">${time}</td></tr></table><p style="margin-top: 16px;">You're welcome to book again at any available time on your dashboard.</p><div style="text-align: center; margin-top: 24px;"><a href="https://www.mantasharkaquatics.net/dashboard" style="display: inline-block; background: #1a2744; color: white; font-weight: 700; padding: 14px 32px; border-radius: 8px; text-decoration: none;">Go to My Dashboard</a></div></div><p style="color: #666; font-size: 13px; text-align: center;">Questions? Reply to this email or chat with us at <a href="https://www.mantasharkaquatics.net">mantasharkaquatics.net</a></p></div>`
    ...
    const dateRows = dl.map((d, i) => `<tr><td style="padding: 6px 0; color: #666;">Lesson ${i + 1}</td><td style="padding: 6px 0; font-weight: 600;">${fmtD(d)}${tl[i] ? ` &middot; ${tl[i]}` : ''}</td></tr>`).join('')
    subject = `Booking Confirmed \u2013 ${dl.length} ${courseName} Lessons`
    html = `<div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; background: #f9f9f9; padding: 32px; border-radius: 12px;"><div style="text-align: center; margin-bottom: 24px;"><h1 style="color: #1a2744; font-size: 24px; margin: 0;">Manta Shark Aquatics</h1></div><div style="background: white; border-radius: 8px; padding: 24px; margin-bottom: 16px;"><h2 style="color: #1a2744; margin-top: 0;">\u2705 Recurring Lessons Confirmed!</h2><p>Hi ${parentName},</p><p>Your recurring lessons have been booked successfully. Here are the details:</p><table style="width: 100%; border-collapse: collapse;"><tr><td style="padding: 8px 0; color: #666;">Student</td><td style="padding: 8px 0; font-weight: 600;">${studentName}</td></tr>${partnerName ? `<tr><td style="padding: 8px 0; color: #666;">Partner</td><td style="padding: 8px 0; font-weight: 600;">${partnerName}</td></tr>` : ''}<tr><td style="padding: 8px 0; color: #666;">Course</td><td style="padding: 8px 0; font-weight: 600;">${courseName}</td></tr><tr><td style="padding: 8px 0; color: #666;">Coach</td><td style="padding: 8px 0; font-weight: 600;">${coachName}</td></tr>${time ? `<tr><td style="padding: 8px 0; color: #666;">Time</td><td style="padding: 8px 0; font-weight: 600;">${time}</td></tr>` : ''}</table><h3 style="color: #1a2744; margin: 20px 0 8px;">Lesson Dates (${dl.length})</h3><table style="width: 100%; border-collapse: collapse;">${dateRows}</table></div><p style="color: #666; font-size: 13px; text-align: center;">Questions? Reply to this email or chat with us at <a href="https://www.mantasharkaquatics.net">mantasharkaquatics.net</a></p></div>`
