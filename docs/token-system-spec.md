...
2. 效期:轉換日 + 60 天
3. 課種 mapping(token 存「可訂課種」,mapping 只在 cron 邏輯):
   - 1-on-1 → 1-on-1;1-on-2 → 1-on-4(業主裁決:降級補位);1-on-4 → 1-on-4;Swim Team → 不轉;trial/assessment → 不適用
...
## 資料模型(草案,實名以 DB 為準)
- token_packages:id/parent_id/course_type_id(可訂課種)/quantity/used_quantity/expires_at/source_credit_package_id(nullable,admin 手動=NULL)/note/created_at
