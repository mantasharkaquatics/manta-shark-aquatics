...
type StudentProgress = {
  student: { id: string; full_name: string; current_level: string; level: { level_number: number; name: string } | null }
  skills: Skill[]
          ...
          {uniqueStudents.map(s => {
            ...
            return (
              ...
              >
                ...
                <span className="text-xs px-2 py-1 rounded-full text-white" style={{ backgroundColor: color + '33', color }}>
                  L{lvl}
                </span>
              </button>
            )
      ...
      {studentData && !loading && (
        <div className="bg-[#111d38] rounded-xl border border-[#1e3a6e] p-5">
          ...
          {studentData.skills.length === 0 ? (
            <p className="text-gray-400 text-sm">此學生尚未分配等級</p>
          ) : (
                      ...
                      {[0, 20, 40, 60, 80, 100].map(v => (
                        <button
                          ...
                          className={`flex-1 py-1 rounded text-xs font-medium transition-all ${
                            pct === v ? 'text-[#1a2744] font-bold' : 'text-gray-400 bg-white/5 hover:bg-white/10'
                          }`}
