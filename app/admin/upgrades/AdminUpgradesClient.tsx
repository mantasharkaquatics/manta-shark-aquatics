...
/**
 * The level settings themselves: assign a level by hand, look up what a level
 * teaches, and read the change log. The queues that used to sit above this --
 * missing progress, progress awaiting publication, coach level requests --
          ...
          {levels.map(l => {
            ...
            return (
              <div key={l.id} className="bg-[#111d38] rounded-xl border border-[#1e3a6e] overflow-hidden">
                ...
                {/* Grouped by stage, not run together. sort_order restarts at 1
                    inside each stage, so a flat numbered list read as scrambled
                    — and the stage is the thing a level is actually taught in. */}
                {isOpen && (
                  <div className="border-t border-[#1e3a6e] px-5 py-4 space-y-4">
                    {STAGES.map(st => {
                      const inStage = lvlSkills.filter(k => Number(k.stage || 1) === st)
