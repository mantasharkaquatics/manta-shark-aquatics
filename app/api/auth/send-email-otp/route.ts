...
export async function POST(req: NextRequest) {
  ...
  try {
    await resend.emails.send({
      ...
      to: normalizedEmail,
      subject: `您的驗證碼：${otpCode}`,
      html: `<div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; background: #f9f9f9; padding: 32px; border-radius: 12px;"><div style="text-align: center; margin-bottom: 24px;"><h1 style="color: #1a2744; font-size: 22px; margin: 0;">Manta Shark Aquatics</h1></div><div style="background: white; border-radius: 8px; padding: 24px; text-align: center;"><p style="color: #666; margin-bottom: 16px;">您的 Email 驗證碼為：</p><div style="font-size: 32px; font-weight: 700; letter-spacing: 0.3em; color: #1a2744; margin-bottom: 16px;">${otpCode}</div><p style="color: #999; font-size: 13px;">此驗證碼將於 10 分鐘後失效。</p></div></div>`,
    })
  } catch (e) {
