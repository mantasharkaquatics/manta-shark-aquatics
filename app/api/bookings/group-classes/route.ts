...
export async function GET(req: NextRequest) {
  ...
  if (date) {
    ...
    const [effList, blocksAll, { data: allSess }] = await Promise.all([
      Promise.all(coachIds.map(id => getEffectiveZones(s, id, date))),
    ...
    if (ids.length > 0) {
      ...
      myTimes = new Set((myB || []).map(b => String(sess.find(x => x.id === b.class_session_id)?.start_time || '').slice(0, 5)))
    }

    const classes: any[] = []
    coachIds.forEach((cid, i) => {
      const eff = effList[i]
      ...
      for (const z of eff.rows) {
        ...
        for (let m = toMin(z.start_time); m + ct.duration_minutes <= toMin(z.end_time); m += 30) {
          ...
          const clash = sess.find(x => x.coach_id === cid && String(x.start_time).slice(0, 5) === t && x.course_type_id !== ct.id && x.enrolled_count > 0)
          if (clash) continue
          const own = sess.find(x => x.coach_id === cid && String(x.start_time).slice(0, 5) === t && x.course_type_id === ct.id)
